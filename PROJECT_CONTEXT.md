...
- `workspaces` (id, name, custom_domain, allowed_widget_domains[])
...
- `conversations` (id, workspace_id, contact_id, channel: chat | email, status: open | snoozed | resolved, assigned_agent_id, last_message_at)
...
`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `POSTMARK_API_KEY`, `POSTMARK_INBOUND_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY`
